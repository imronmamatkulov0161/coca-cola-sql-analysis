...
-- 3.5 Maximum drawdown during the 2020 COVID crash
SELECT ROUND((1 - MIN(close)/MAX(close)) * 100, 1) AS max_drawdown_pct
FROM coca_cola_stock
WHERE date BETWEEN '2020-01-01' AND '2020-04-30';

-- 3.6 Recovery date — first close back above the pre-crash peak
SELECT MIN(date) AS recovery_date
FROM coca_cola_stock
WHERE date > '2020-03-23'
  AND close >= (SELECT MAX(close)
                FROM coca_cola_stock
                WHERE date BETWEEN '2020-01-01' AND '2020-02-21');
...
-- ============================================================
-- SECTION 5: SUPPLEMENTARY CORPORA
-- Sentiment and rating distributions (Yelp, IMDB)
-- ============================================================

-- 5.1 Monthly sentiment summary by country (highest positive share)
SELECT 
    b.country,
    b.year,
    b.month,
...
-- ============================================================
-- SECTION 6: CROSS-TABLE — SENTIMENT vs STOCK (2022–2023)
-- Tweets are aggregated per day first to avoid join fan-out
-- (row-level join would weight prices by tweet volume)
-- ============================================================

-- 6.1 Monthly positive-tweet share vs average closing price
SELECT
    EXTRACT(YEAR FROM s.date)  AS year,
    EXTRACT(MONTH FROM s.date) AS month,
    ROUND(AVG(s.close)::numeric, 2) AS avg_close,
    SUM(d.tweets) AS tweets,
    ROUND(SUM(d.positive) * 100.0 / SUM(d.tweets), 1) AS positive_pct
FROM coca_cola_stock s
JOIN (
    SELECT created_at::date AS tweet_date,
           COUNT(*) AS tweets,
           SUM(CASE WHEN sentiment = 'positive' THEN 1 ELSE 0 END) AS positive
    FROM tweets
    GROUP BY created_at::date
) d ON d.tweet_date = s.date
GROUP BY year, month
ORDER BY year, month;
